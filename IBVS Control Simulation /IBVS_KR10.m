...
[C, error_over_time, x_over_time, y_over_time, z_over_time, time_array, time] = IBVS_outer_loop(q, KR5, Reference_Photo, Current_Photo, dots, C, reference_position, Zref, maxDelta, Kp, errorThreshold, maxIterations, dt_control, dt_kin,cam,error_over_time, time_array, x_over_time, y_over_time, z_over_time);
...
function [C, error_over_time, x_over_time, y_over_time, z_over_time, time_array, time] = IBVS_outer_loop(q, KR5, Reference_Photo, Current_Photo, dots, C, reference_position, Zref, maxDelta, Kp, errorThreshold, maxIterations, dt_control, dt_kin,cam, error_over_time, time_array, x_over_time, y_over_time, z_over_time)
    ...
    while iteration < maxIterations
        ...
        velocity_joint=pinv(J)*controlInput;
        disp(velocity_joint)
        ...
        disp(['Kinematics Update Iteration ', num2str(iteration), ...
              ', Sum of Absolute Error: ', num2str(sum(abs(error_im(:))))]);
        
        % Reset the timer for the next kinematics update
        lastKinematicUpdate = tic;
    ...
    while toc(lastKinematicUpdate) < dt_kin
        ...
        [Current_Photo,cam] = update_camera(dots,C);
         ...
         % Compute the size of the square (assumed from first two points)
            ...
            disp('L_total, Lpinv, error_im(:)')
            disp(L_total)
            disp(pinv(L_total))
            disp(error_im(:))
            % Compute the control input from the image error and Jacobian.
            % In the original code, only selected components were used.
            controlInput = -Kp * (pinv(L_total) * error_im(:));
            disp(controlInput)
            % (This inner loop sends velocity commands at high-frequency.
            %  In a real application, this command could be sent to native low-level controllers.)
